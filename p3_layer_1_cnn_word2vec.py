...
def make_dataset(model_word_embed, 
    ...
    for f in range(0, len(files)):
        # train 
        filename = open(train_dir + files[f], 'r').read().splitlines()
    ...
    print("x_train:",x_train.shape," - y_train:",y_train.shape, "- train_questions:",len(train_questions))
    print("x_test:",x_test.shape," - y_test:",y_test.shape, "- test_questions:",len(test_questions))
    return x_train , y_train , x_test , y_test , train_questions , test_questions
...
x_train , y_train , x_test , y_test , train_questions , test_questions = make_dataset(w2v,word_embed_dim,maxim)
model = build_2_model(input_shape=(32,300,1))
...
acc , error_df = test_accuracy(x_test,y_test,test_questions)
